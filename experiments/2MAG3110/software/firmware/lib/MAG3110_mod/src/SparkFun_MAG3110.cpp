...
uint8_t MAG3110::readRegister(uint8_t address) {
    ...
    if (port == 0) {
        ...
        Wire1.beginTransmission(MAG3110_I2C_ADDRESS);
        Wire1.write(address);
    ...
    if (port == 0) {
        ...
        Wire1.requestFrom(MAG3110_I2C_ADDRESS, 1);
...
void MAG3110::readMag(int *x, int *y, int *z) {
    ...
    if (port == 0) {
        ...
        // Start readout at X MSB address
        Wire1.beginTransmission(MAG3110_I2C_ADDRESS);
        Wire1.write(MAG3110_OUT_X_MSB);
        Wire1.endTransmission();

        delayMicroseconds(2);

        // Read out data using multiple byte read mode
        Wire1.requestFrom(MAG3110_I2C_ADDRESS, 6);

        for (uint8_t idx = 0; idx <= 2; idx++) {
            values[idx] = Wire1.read() << 8;    // MSB
            values[idx] |= Wire1.read();            // LSB
        }
    }
...
void MAG3110::calibrate() {
    ...
    if (millis() > 5000 &&
        millis() - timeLastChange > CALIBRATION_TIMEOUT) //If the timeout has been reached, exit calibration
        exitCalMode();
}
